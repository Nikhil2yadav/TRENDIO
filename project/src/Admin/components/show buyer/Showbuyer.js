...
const Showbuyer = () => {
  ...
  const fetchBuyers = async () => {
    try {
      const response = await axios.get('http://trendio.free.nf/api/GetBuyers.php');
  ...
  const deleteBuyer = async (BuyerId) => {
    ...
    if (confirmed) {
      try {
        await axios.get(`http://trendio.free.nf/api/DeleteBuyerApi.php?buyerId=${BuyerId}`);
  ...
  const toggleBuyerStatus = async (BuyerId, currentStatus) => {
    try {
      ...
      const response = await axios.post(
        'http://trendio.free.nf/api/UpdateBuyerStatus.php',
        formData
      );
