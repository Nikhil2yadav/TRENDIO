...
//         <h6 className="text-danger text-end">Grand Total: <FontAwesomeIcon icon={faInr} />{usersData.reduce((acc, item) => acc + Number(item.subtotal), 0)}</h6>
...
//             Order Status:
//           </label>
//           <select className="form-select form-control shadow-sm border-primary" id="orderStatus">
...
const ShippedOrder = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        const response = await axios.post(
          'http://localhost:8080/college%20project/mini%20project/api/GetDetailinformationaboutshippedorder.php',
          { orderDetailId },
          { headers: { 'Content-Type': 'application/x-www-form-urlencoded' }}
        );
        if (Array.isArray(response.data)) {
