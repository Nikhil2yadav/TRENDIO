...
const Ratingfeedback = () => {
    ...
    const fetchFeedback = async () => {
      try {
        const response = await axios.get(
          `http://trendio.free.nf/api/GetFeedbackSelleraccording.php?SellerId=${SellerId}`
        );
  ...
  const toggleFeedbackStatus = async (feedbackId, currentStatus) => {
    try {
      ...
      const response = await axios.post(
        "http://trendio.free.nf/api/UpdateFeedbackStatus.php",
        formData
      );
  ...
  //     if (response.data.success) {
  //       setUsersData((prevData) =>
  //         prevData.filter((feedback) => feedback.feedbackid !== feedbackId)
  //       );
